...
namespace ZetScript{
	...
	*stkCurrentData++={NULL,var_ref,INS_PROPERTY_SCRIPTVAR};
	...
	CVirtualMachine::CVirtualMachine(ZetScript *_zs){
	...
	std::string CVirtualMachine::stk_C_TypeStr(const StackElement & stk_v){
		if(stk_v.properties & STK_PROPERTY_TYPE_INTEGER){
			...
			ScriptClass *c = GET_SCRIPT_CLASS(((CScriptVar *)(stk_v.varRef))->idx_class);
	...
	const ScriptFunction * CVirtualMachine::getCurrent_C_FunctionCall(){
	...
	void CVirtualMachine::buildCache(){
		...
		std::vector<ScriptFunction *> *vec_script_function_object_node_aux=script_function_factory->getVectorScriptFunctionNode();
		size_vec_script_function_object_node=vec_script_function_object_node_aux->size();
		vec_script_function_node=(ScriptFunction **)malloc(sizeof(ScriptFunction *)*size_vec_script_function_object_node);
...
//		ScriptClass *script_class_aux;
		...
		ByteCode *start_it=info_function->instruction;
		ByteCode *instruction_it=start_it;
		...
		//
		 ...
		 for(;;){ // foreach asm instruction ...
			...
			ByteCode * instruction = instruction_it++;
				...
				if(op1_value==LOAD_TYPE::LOAD_TYPE_VARIABLE || operator_type==VGET){
					if(operator_type==VGET){
						...
						case INS_PROPERTY_THIS_SCOPE:
							...
							if(scope_type == INS_PROPERTY_ACCESS_SCOPE){
								...
								ByteCode *previous_ins= (instruction-1);
					...
					if(scope_type == INS_PROPERTY_ACCESS_SCOPE){
						...
						if(stk_ins->properties & STK_PROPERTY_TYPE_SCRIPTVAR){
							...
							//ScriptClass *sc =GET_SCRIPT_CLASS(((CScriptVar *)class_obj)->idx_class);
						...
						function_obj =(ScriptFunction *)si->object.stkValue;
				...
				// 1. get function object ...
			 	 {
					...
					aux_function_info = NULL;//(ScriptFunction *)callAle->stkValue;
					...
					if(((callAle)->properties & STK_PROPERTY_IS_INSTRUCTIONVAR)){// || deduce_function){
						ByteCode *callAleInstruction = (ByteCode *)(callAle)->stkValue;
						...
						{
							std::vector<ScriptFunction *> *vec_global_functions=&(main_function_object->local_function);
						...
						if(((callAle)->properties & STK_PROPERTY_UNRESOLVED_FUNCTION)==0){
							aux_function_info=(ScriptFunction *) (callAle->stkValue);
						}
					}
			...
			write_error(INSTRUCTION_GET_FILE_LINE(info_function,instruction),"operator type(%s) not implemented",ByteCode::opCodeToStr(instruction->op_code));
			RETURN_ERROR;
