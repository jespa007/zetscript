...
namespace ZetScript{
	...
	short 		 ScriptFunction::INSTRUCTION_getLine(ByteCode * ins){
	...
	const char *  ScriptFunction::INSTRUCTION_getSymbolName(ByteCode * ins){
		OpCodeInstructionSourceInfo *info=getInstructionInfo(ins);
	...
	const char * ScriptFunction::INSTRUCTION_getFile(ByteCode * ins){
	...
	VariableSymbolInfo *	ScriptFunction::registerVariable(const std::string & file, short line, const std::string & variable_name, const std::string & c_type, intptr_t ref_ptr, unsigned short symbol_info_properties)
	{
		VariableSymbolInfo *vsi=ScriptClassBase::registerVariable(file,line,this->symbol_info.symbol->idxScope,  variable_name,  c_type,  ref_ptr,   symbol_info_properties);
