...
namespace ZetScript{
	...
	std::string * ScriptEval::getCompiledSymbol(const std::string & s){
	...
	bool ScriptEval::eval(const char * str, const char *  _filename, int _line){
		int line =_line;
	...
	void ScriptEval::iniVars(){
		...
		defined_keyword[KEYWORD_TYPE::VAR_KEYWORD] = {VAR_KEYWORD,"var",new KeywordFunction(std::bind(&ScriptEval::evalKeywordVar,this,_1,_2,_3,_4))};
		defined_keyword[KEYWORD_TYPE::IF_KEYWORD] = {IF_KEYWORD,"if",new KeywordFunction(std::bind(&ScriptEval::evalKeywordIf,this,_1,_2,_3,_4))};
		defined_keyword[KEYWORD_TYPE::ELSE_KEYWORD] = {ELSE_KEYWORD,"else"};
		defined_keyword[KEYWORD_TYPE::FOR_KEYWORD] = {FOR_KEYWORD,"for",new KeywordFunction(std::bind(&ScriptEval::evalKeywordFor,this,_1,_2,_3,_4))};
		defined_keyword[KEYWORD_TYPE::WHILE_KEYWORD] = {WHILE_KEYWORD,"while",new KeywordFunction(std::bind(&ScriptEval::evalKeywordWhile,this,_1,_2,_3,_4))};
		defined_keyword[KEYWORD_TYPE::DO_WHILE_KEYWORD] = {DO_WHILE_KEYWORD,"do",new KeywordFunction(std::bind(&ScriptEval::evalKeywordDoWhile,this,_1,_2,_3,_4))}; // while is expected in the end ...

		defined_keyword[KEYWORD_TYPE::SWITCH_KEYWORD] = {SWITCH_KEYWORD,"switch",new KeywordFunction(std::bind(&ScriptEval::evalKeywordSwitch,this,_1,_2,_3,_4))};
		...
		defined_keyword[KEYWORD_TYPE::RETURN_KEYWORD] = {RETURN_KEYWORD,"return",new KeywordFunction(std::bind(&ScriptEval::evalKeywordReturn,this,_1,_2,_3,_4))};
	...
	ScriptEval::ScriptEval(ZetScript *_zs){
		//CURRENT_PARSING_FILE_IDX=-1;
		current_parsing_file=NULL;
		pCurrentFunctionInfo=NULL;
		this->zs=_zs;
	...
	bool   ScriptEval::endExpression(const char * s){
		return *s==')' || *s==','||  *s==']' ||  *s==']' ||  *s==';' || *s == 0;
	}

	ScriptEval::SEPARATOR_TYPE   ScriptEval::evalSeparator(const char *s){
	...
	ScriptEval::OPERATOR_TYPE   ScriptEval::evalOperator(const char *s){
	...
	ScriptEval::PRE_OPERATOR_TYPE   	ScriptEval::evalPreOperator(const char *s){
	...
	ScriptEval::IDENTITY_OPERATOR_TYPE   ScriptEval::evalPrePostIdentityOperator(const char *s){
	...
	ScriptEval::KEYWORD_TYPE ScriptEval::isKeyword(const char *c){
	...
	ScriptEval::DIRECTIVE_TYPE ScriptEval::isDirective(const char *c){
		// PRE: The first char must be starting symbol.
		char *str=(char *)c;
	...
	char * ScriptEval::evalLiteralNumber(const char *c, int & line, std::string & value, bool & error){
	...
	bool ScriptEval::isAccessPunctuator(char *s){
		return *s=='.' || *s=='[' || *s=='(';
	}

	bool ScriptEval::isEndSymbolToken(char *s, char pre){
	...
	// to std::string utils ...
	char * ScriptEval::evalSymbol(const char *start_word, int line,ScriptEval::tTokenNode * token_node){
		// PRE:
	...
	char *  ScriptEval::getIdentifierToken(const char *s, std::string & symbol){
	...
	bool ScriptEval::isLiteral(const std::string & symbol){
		return symbol=="true" || symbol == "false" || symbol=="undefined" || symbol == "null";
	}

	bool ScriptEval::isIdentifierNameExpressionOk(const std::string & symbol, int line){
	...
	//------------------------------------------------------------------------------------------------------------
	char * ScriptEval::evalFunctionObject(const char *s,int & line,  Scope *scope_info, std::vector<OpCodeInstructionEval> 		*	instruction){
	...
	char * ScriptEval::evalStructObject(const char *s,int & line,  Scope *scope_info, std::vector<OpCodeInstructionEval> 		*	instruction){
	...
	char * ScriptEval::evalVectorObject(const char *s,int & line,  Scope *scope_info,  std::vector<OpCodeInstructionEval> *	instruction){
	...
	char * ScriptEval::evalNewObject(const char *s,int & line,  Scope *scope_info, std::vector<OpCodeInstructionEval> 		*	instruction){
		// PRE: **ast_node_to_be_evaluated must be created and is i/o ast pointer variable where to write changes.
		char *aux_p = (char *)s;
		std::string symbol_value;
	...
	OP_CODE ScriptEval::puntuator2instruction(OPERATOR_TYPE op){
	...
	bool ScriptEval::makeOperatorPrecedence(std::vector<tTokenNode> * vExpressionTokens,std::vector<OpCodeInstructionEval> *instruction, int idx_start,int idx_end, bool & error){
	...
	char * ScriptEval::evalExpression(const char *s, int & line, Scope *scope_info, std::vector<OpCodeInstructionEval> 	* instruction){
				...
				if(isAccessPunctuator(aux_p) || symbol_token_node.token_type == TOKEN_TYPE::IDENTIFIER_TOKEN){
					...
					while(isAccessPunctuator(aux_p)){
						...
						case '[': // std::vector access
							...
							//symbol_token_node.instruction.push_back(ByteCode(OP_CODE::VGET));
						...
						case '.': // member access
							...
							//pCurrentFunctionInfo->function_info_object->instruction_info[symbol_token_node.instruction.size()]=ScriptFunction::OpCodeInstructionSourceInfo(accessor_value,current_parsing_file,line);
							//symbol_token_node.instruction.push_back(ByteCode(OP_CODE::LOAD)); //--> must add symbol value instruction...
	...
	char * ScriptEval::evalKeywordDelete(const char *s,int & line,  Scope *scope_info, bool & error){
		// PRE: **ast_node_to_be_evaluated must be created and is i/o ast pointer variable where to write changes.
		char *aux_p = (char *)s;
		std::string symbol_value;
		KEYWORD_TYPE key_w;
	...
	char * ScriptEval::isClassMember(const char *s,int & line,ScriptClass **sc,std::string & member_symbol, bool & error){
	...
	char * ScriptEval::evalKeywordClass(const char *s,int & line, Scope *scope_info, bool & error){
		// PRE: **ast_node_to_be_evaluated must be created and is i/o ast pointer variable where to write changes.
		char *aux_p = (char *)s;
	...
	char * ScriptEval::evalKeywordFunction(const char *s,int & line,  Scope *scope_info, bool & error){
	...
	char *  ScriptEval::evalKeywordReturn(const char *s,int & line,  Scope *scope_info, bool & error){
		// PRE: **ast_node_to_be_evaluated must be created and is i/o ast pointer variable where to write changes.
		char *aux_p = (char *)s;
		KEYWORD_TYPE key_w;
		std::string s_aux;
	...
	char * ScriptEval::evalKeywordWhile(const char *s,int & line, Scope *scope_info,  bool & error){
	...
	char * ScriptEval::evalKeywordDoWhile(const char *s,int & line, Scope *scope_info,  bool & error){
	...
	char * ScriptEval::evalKeywordIf(const char *s,int & line,  Scope *scope_info, bool & error){
	...
	char * ScriptEval::evalKeywordFor(const char *s,int & line,  Scope *scope_info,  bool & error){
	...
	char * ScriptEval::evalKeywordSwitch(const char *s,int & line,  Scope *scope_info,  bool & error){
		...
		if(key_w != KEYWORD_TYPE::UNKNOWN_KEYWORD){
			if(key_w == KEYWORD_TYPE::SWITCH_KEYWORD){
				...
				if(*aux_p == '('){
						...
						if((aux_p = ScriptEval::evalExpression(
								aux_p,
								line,
								scope_info,
	...
	char * ScriptEval::evalKeywordVar(const char *s,int & line,  Scope *scope_info, bool & error){
	...
	char * ScriptEval::evalBlock(const char *s,int & line,  Scope *scope_info, bool & error){
		// PRE: **ast_node_to_be_evaluated must be created and is i/o ast pointer variable where to write changes.
		char *aux_p = (char *)s;
	...
	char *ScriptEval::evalBreak(const char *s, int & line, Scope *scope_info, bool & error){
	...
	char *ScriptEval::evalContinue(const char *s, int & line, Scope *scope_info, bool & error){
		// TODO: "find findConditionForBreak if current stackBreakForWhileSwitch.size() > 0\n"
		char *aux_p=(char*)s;
		{ // ok break is valid in current scope...
	...
	char *ScriptEval::evalDefaultCase(const char *s, int & line, Scope *scope_info,  bool & error){
		char *aux_p=(char *)s;
	...
	char *ScriptEval::evalKeyword(const char *s, int & line, Scope *scope_info, bool & error){
		...
		if(keyw != KEYWORD_TYPE::UNKNOWN_KEYWORD){ // a keyword was detected...
			...
			case FUNCTION_KEYWORD:
				...
				return NULL;
			case CLASS_KEYWORD:
				if((aux_p = ScriptEval::evalKeywordClass(s,line,scope_info,error)) != NULL){
					return aux_p;
				}
				error = true;
				return NULL;
	...
	char * ScriptEval::eval_Recursive(const char *s, int & line, Scope *scope_info,  bool & error){
				...
				if(directive != DIRECTIVE_TYPE::UNKNOWN_DIRECTIVE){
					...
					case INCLUDE_DIRECTIVE:
...

						{
							...
							std::string current_file_str=ScriptEval::current_parsing_file;
							//int current_file_idx=ScriptEval::CURRENT_PARSING_FILE_IDX;
							...
							//ScriptEval::CURRENT_PARSING_FILE_IDX=current_file_idx;
						}
	...
	void ScriptEval::popFunction(){
		...
		for(unsigned i=0; i < pCurrentFunctionInfo->instruction.size(); i++){
			...
			if(ls->idxScriptFunction != ZS_UNDEFINED_IDX){ // solve first symbol first access...
				...
				ScriptFunction *sf=GET_SCRIPT_FUNCTION(ls->idxScriptFunction);
				...
				if(instruction->properties & INS_PROPERTY_THIS_SCOPE){ // trivial this.
					...
					Scope *scope=GET_SCOPE(ls->idxScope);
