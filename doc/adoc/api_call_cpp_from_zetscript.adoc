...
=== Call C++ from ScriptEngine

To call C&#43;&#43; code from _ScriptEngine_ it has to create and register a C function with a particular prototype. That C function acts as interface between ScriptEngine and C&#43;&#43;code. 

The function must always include _zetscript::ScriptEngine *_ as the FIRST parameter and from SECOND till a maximmum of 9 parameters as a xref:api_types#_types[primitive type] or a xref:api_expose_cpp_types#_register_a_type[registered type]. The function it can return nothing or it can return a xref:api_types#_types[primitive type] or a xref:api_expose_cpp_types#_register_a_type[registered type].

*Syntax*

[source,cpp]
----
ReturnType zetscriptCppInterface(ScriptEngine *_script_engine, ParamType *_arg1, ParamType *_arg2, ..., ParamType *_arg9 ){
	....
}
----
...
After the C function is created is registered with xref:api_types#&#95;_zetscript_registerfunction[ScriptEngine::registerFunction]. 
...
This section will explain how to register C functions that return the following ScriptEngine types,
...
To register a C function that returns a xref:language_data_types.adoc#&#95;string[String] it can specify _StringScriptObject &#42;_ as return type. The instantiation of _StringScriptObject &#42;_ it&#39;s done throught _ScriptEngine::newStringScriptObject_ using ScriptEngine context.
