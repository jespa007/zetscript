...
The function must always include _zetscript::ZetScript *_ as the FIRST parameter and from SECOND till a maximmum of 9 parameters as a xref:api_types#_types[primitive type] or a xref:api_expose_cpp_types#_register_a_type[registered type]. The function it can return nothing or it can return a xref:api_types#_types[primitive type] or a xref:api_expose_cpp_types#_register_a_type[registered type].
...
After the C function is created is registered with xref:api_types#&#95;_zetscript_registerfunction[ZetScript::registerFunction]. 
...
[source,cpp]
----
include::../examples/api/register_function/register_function.cpp[]
----
...
[source,txt]
----
include::../examples/api/register_function/register_function_out.txt[]
----
...
To register a C function that returns a xref:language_data_types.adoc#&#95;string[String] it can specify _StringObject &#42;_ as return type. The instantiation of _StringObject &#42;_ it&#39;s done throught _ZetScript::newStringObject_ using ZetScript context.
...
To register a C function that returns a xref:language_data_types.adoc#lowbar;array[Array] it must specify _ArrayObject &#42;_ as return type. The instantiation of _ArrayObject &#42;_ it's done through _ZetScript::newArrayObject_ using ZetScript context.
...
To register a C function that returns a xref:language_data_types.adoc#lowbar;object[Object] it must specify _ObjectObject &#42;_ as return type. The instantiation of _ObjectObject &#42;_ it's done through _ZetScript::newObjectObject_ using ZetScript context.
...
To register a C function that returns a xref:api_expose_cpp_types#_register_type[registered type] it must specify _ClassObject &#42;_ as return type. To instantiate _ClassObject &#42;_  is done through _ZetScript::newClassObject_ using ZetScript context and the registered class instantiated.

A difference from returning a pointer type of a registered type, when the C function returns a _ClassObject &#42;_ ZetScript is in charge to deallocate automatically when it has no more references to it. 

====
The action of return a _ClassObject &#42;_ ZetScript it considers the instance of a new object in the script code so it needs to register the type as INSTANTIABLE (See xref:api_expose_cpp_types#&#x5f;register_a_type_an_instantiable[section 3.5.1] to know how to register a type AS INSTANTIABLE)
====
...
To register a C function that accepts a xref:language_data_types.adoc#_string[String] it can specify _const char *_ , _String *_  or _StringObject *_ as a parameter type,
...
To register a C function that accepts a xref:language_data_types.adoc#lowbar;array[Array] it must specify _ArrayObject &#42;_ as parameter type.
...
To register a C function that accepts a xref:language_data_types.adoc#lowbar;object[Object] it must specify _ObjectObject &#42;_ as parameter type.
...
To register a C function that accepts a xref:language_data_types.adoc#lowbar;function[Function] it must specify _Function &#42;_ as parameter type. To make  _Function_ callable from C++ must use _ZetScript::bindFunction_ (see xref:_api_call_zetscript_from_cpp#&#x5f;call_zetscript_from_c[section 3.4] to know more about ZetScript functions binding)
