...
After the C function is created, it has to registered with xref:api_data_types&#95;scriptengine&#95;registerfunction[ScriptEngine::registerFunction]. 
...
To register a C function that returns a <<language_data_types.adoc#\_boolean,Boolean>> it must specify _bool_ as return type.
...
To register a C function that returns a <<language_data_types.adoc#\_integer,Integer>> it must specify _zs_int_ as return type.
...
To register a C function that returns a <<language_data_types.adoc#\_float,Float>> it must specify _zs_float_ as return type.
...
To register a C function that returns a <<language_data_types.adoc#\_string,String>> it can specify _String_ as return type.
...
To register a C function that returns a <<language_data_types.adoc#\_string,String>> it can specify <<api_data_types.adoc#_stringscriptobject[zetscript::StringScriptObject] pointer as return type. The instantiation of _StringScriptObject_ is done throught ScriptEngine instance.
...
To register a C function that returns a <<language_data_types.adoc#\_array,Array>> it must specify _ArrayScriptObject &#42;_ as return type. The instantiation of _ArrayScriptObject &#42;_ it's done through _ScriptEngine::newArrayScriptObject_ using ScriptEngine instance.
...
To register a C function that returns a xref:language_data_types.adoc#lowbar;object[Object] it must specify _ObjectScriptObject &#42;_ as return type. The instantiation of _ObjectScriptObject &#42;_ it's done through _ScriptEngine::newObjectScriptObject_ using ScriptEngine instance.
...
To register a C function that returns a registered type it must return _ClassObject &#42;_. To instantiate _ClassObject &#42;_  is done through _ScriptEngine::newClassObject_ using ScriptEngine instance and the registered class instantiated.
...
To register a C function that accepts a xref:language_data_types.adoc#&95;boolean[Boolean] it must specify _bool &#x2a;_ as parameter type.
...
To register a C function that accepts a xref:api_expose_cpp_types&#95;register_type[registered type] as a parameter, it must specify native pointer type as parameter type.
