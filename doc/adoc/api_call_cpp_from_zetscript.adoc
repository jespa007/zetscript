...
To register a C function that returns a <<language_data_types.adoc#_object,Object>> it must specify __zetscript::ObjectScriptObject *__ as return type. The instantiation of __zetscript::ObjectScriptObject *__ it's done through _ScriptEngine::newObjectScriptObject_ using ScriptEngine instance.
