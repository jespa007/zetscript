...
To register a C function that returns a xref:api_expose_cpp_types#_register_type[registered type] it must specify _ClassObject &#42;_ as return type. To instantiate _ClassObject &#42;_  is done through _ScriptEngine::newClassObject_ using ScriptEngine instance and the registered class instantiated.
