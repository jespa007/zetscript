...
To call C++ code from _ZetScript_ it has to create and register C function as it describes <<api_data_types#_scriptengineregisterfunction,ScriptEngine::registerFunction>>. 
...
To register a C function that returns a <<language_data_types.adoc#_string,String>> it can specify <<api_data_types.adoc#_stringscriptobject,zetscript::StringScriptObject>> pointer as return type. The instantiation of _zetscript::StringScriptObject_ is done throught ScriptEngine instance.
...
To register a C function that returns a xref:api_expose_cpp_types.adoc#_register_type[registered type] it can be done by specifying the following return types,
...
To register a C function that returns a xref:api_expose_cpp_types.adoc#_register_type[registered type] it must specify its native pointer type as return.
...
To register a C function that returns a registered type it must return __zetscript::ClassScriptObject *__. To instantiate __zetscript::ClassScriptObject *__  is done through _ScriptEngine::newClassObject_ using ScriptEngine instance and the registered class instantiated.
...
====
The action of return a __zetscript::ClassScriptObject *__ ZetScript it considers the instance of a new object in the script code so it needs to register the type as INSTANTIABLE (See xref:api_expose_cpp_types#&#x5f;register_a_type_an_instantiable[section 3.5.1] to know how to register a type AS INSTANTIABLE)
====
...
To register a C function that accepts a <<language_data_types.adoc#_boolean,Boolean>> it must specify _bool &#x2a;_ as parameter type.
...
To register a C function that accepts a <<language_data_types.adoc#_integer,Integer>> it can specify __zetscript::zs_int__ or _zs_int &#x2a;_ as parameter type.
...
To register a C function that accepts a <<language_data_types.adoc#_float,Float>> it must specify _zs_float &#x2a;_ as parameter type.
...
To register a C function that accepts a <<language_data_types.adoc#_string,String>> it can specify _const char *_ , _String *_  or _StringScriptObject *_ as a parameter type,
...
To register a C function that accepts a <<language_data_types.adoc#_function,Function>> it must specify _Function &#42;_ as parameter type. To make  _Function_ callable from C++ must use _ScriptEngine::bindScriptFunction_ (see <<api_call_zetscript_from_cpp#_call_zetscript_from_c,section 3.4>> to know more about ZetScript functions binding)

*Example*

The following example it registers function _paramFunction_ that accepts a <<language_data_types.adoc#_function,Function>> as parameter. Later, it evaluates an script that calls _paramFunction_ passing anonymous function. In the calling function _paramFunction_ it binds the anonymous function and it calls.
...
In the following example it registers the _Number_ type (see <<api_expose_cpp_types.adoc#_register_type,section 3.5.1>> to know how to register a type) and the property _Number::value_ (see <<api_expose_cpp_types.adoc#_properties,section 3.5.5>> to know how to register a property) and it registers function _mul10Number_ that accepts a _Number &#x2a;_ type as parameter. In the script execution it instances a _Number_ and it is passed as argument to _mul10Number_ that multiplies its property _value_ by 10.
