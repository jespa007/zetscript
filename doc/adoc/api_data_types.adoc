...
_ClassObject_ it a subclass of _ObjectScriptObject_ that implements the _class_ type defined in ZetScript. Also it is used as a wrapper for instanced of registered C++ types.
...
ZetScript::registerFunction is used to call C&#43;&#43; code from _ZetScript_. The function needs a C function with a particular prototype to connects both sides (i.e native and script). 
...
[source,cpp]
----
ReturnType (ZetScript *_zs, ParamType *_arg1, ParamType *_arg2, ..., ParamType *_arg9 )
----
...
As a parameters, the function must always include _zetscript::ZetScript *_ as the FIRST parameter and from SECOND till a maximmum of 9 parameters type _ParamType_ as a registered type or one of the following types,
...
To see more and complete examples see xref:api_call_cpp_from_zetscript#_call_c_from_zetscript[Call C++ from ZetScript]
...
_ZetScript::bindScriptFunction_ it binds ZetScript function by the C&#43;&#43; function signature and the symbol name or _Function_ object.

*Syntax*

[source,cpp]
----
 ZetScript::bindScriptFunction<F>(const String & _script_function_name);
...
[.underline]#Parameters#
 ...
 - __script_function_: _ZetScript::String_ or _Function *_ as the name or the script function reference to be registered.
