...
__zetscript::zs_float__ type it defines a float variable represented as IEEE-754 floating point numbers in 32-bit or 64 bit it depending whether ZetScript is compiled for 32bits or 64bits.
...
|It tells that the _StackElement_ is a __Float__ representing its value as *zs_float &#42;*

|STACK_ELEMENT_PROPERTY_BOOL
|It tells that the _StackElement_ is a _Boolean_ representing its value as __zetscript::zs_int__

|STACK_ELEMENT_PROPERTY_SCRIPT_TYPE_ID
|It tells that the _StackElement_ is a script_type_id_ of the  _ScriptType_ representing its value by a __zetscript::zs_int__ 
...
- __return_script_type_id__:  return script type id.
- __owner_script_type_id__: script type id  which script function it belongs
- __properties__: Script function properties
...
[.underline]#Parameters#
 ...
 - ___stack_size__: Set stack size used by the virtual machine (default 256)
...
[source,cpp]
----
include::../examples/api/types/script_engine_constructor.cpp[]
----
...
Binds script function to be callable from C{plus}{plus}.
...
[.underline]#Parameters#
 ...
 - ___script_function_name__: Bind script function by script function name to bind
 - ___member_function_script_object__: Script member function instance
 - ___object__: Script object instance owner of the script member function
 - ___script_function__: Script function instance
...
====
*Note* : The method do not supports (by now) a forward register to an extended types when any member o property is being registered on  _B_. So, it is important to register *FIRST* all members and properties of _B_ *BEFORE* before apply the extension to _T_.
====
...
The header describes the following,
 ...
 - __Function__: The function name. The main function is described as @MainFunction.
 - __Stack code__: Stack required for code
 - __Stack local vars__: Stack required for local variables
 - __Total stack required__: Total stack required
 - __Scopes__: Total scopes

*Instructions*

Below the header it appears a list of the instructions of generated code with the following columns,

- __NUM__: Instruction number
- __RS__: Required Stack for current instruction
- __AS__: Acumulated Stack in the current instruction
- __BYTE CODE__: Description of the Byte code name used and operands
...
Pushes stack element to stack to be readed later as a return function in ZetScript. The order how stack elements are pushed/popped is FIFO (First into stack First out to be processed). As a difference from register normal function seen in <<api_data_types.adoc#_scriptengineregisterfunction,ScriptEngine::registerFunction>> this operation allows return multiple values.
...
[source,cpp]
----
include::../examples/api/types/script_engine_push_stack_element.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_push_stack_element_out.txt[]
----
...
- ___const_name__	: Constant name
- ___value__	: Boolean value
- ___registered_file__ : Source file where the function was registered
- ___registered_line__ : Line file where the function was registered
...
[source,cpp]
----
include::../examples/api/types/script_engine_register_constant_boolean.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_register_constant_boolean_out.txt[]
----
...
[source,cpp]
----
include::../examples/api/types/script_engine_register_constant_float.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_register_constant_float_out.txt[]
----
...
[source,cpp]
----
include::../examples/api/types/script_engine_register_constant_integer.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_register_constant_integer_out.txt[]
----
...
[source,cpp]
----
include::../examples/api/types/script_engine_register_constant_string.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_register_constant_string_out.txt[]
----
...
- ___c_function__: A C function that returns a type as a value of the property with the following signature,
...
- ___c_function__: C function reference to be registered
...
- ___function_name__ : Function name.
- ___c_function__ : A C function to register with the following signature,
...
To see more and complete examples see <<api_call_cpp_from_zetscript.adoc#_call_c_from_zetscript[Call C{plus}{plus} from ZetScript]>>
...
- ___member_function_name__	: Member function name
- ___c_function__: A C function with the following signature,
...
- ___function_name__ : The function name to be refered in ZetScript. A static member function can be defined as metamethod if ___function_name__ matches some of the name of the metamethod seen at xref:language_class.adoc#_static_metamethods[The language.Class.Member Functions. Static metamethods]
...
ScriptType * registerType<T>(
	...
	, F _new_c_function=NULL
	, F _delete_c_function=NULL
);
----
...
- ___script_type_name__: The script type name.
- ___new_c_function__ (Optional):  It defines the c function that creates the native type when an instance of this type is created in ZetScript (Optional if it's instantiable). The signature of the c function is,
...
- ___delete_c_function__ : It defines the c function that destroys the native type when a instance of this type is destroyed in ZetScript (Optional if it's instantiable). The signature of the c function is,
...
Register as non instantiable type it means that it *CANNOT* create or instantiate new objects of that _type_ in ZetScript using the _new_ operator, so the object only can be acceded by reference returned by some registered function. This kind of register is useful when it wants to have a control of instantiated objects in the native application.

If for example we perform the following code,

[source,cpp]
----
include::../examples/api/types/script_engine_register_type_non_instantiable_error.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_register_type_non_instantiable_error_out.txt[]
----  
...
[source,txt]
----
include::../examples/api/types/script_engine_register_type_non_instantiable_out.txt[]
----
...
Register as instantiable type it means that it *CAN* instantiate objects of that 'type' in ZetScript code using operator __new__.
...
[source,cpp]
----
include::../examples/api/types/script_engine_register_type_instantiable.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_register_type_instantiable_out.txt[]
----
...
- ___script_object__: Script object to be dereferenced.
...
[source,cpp]
----
include::../examples/api/types/script_engine_stack_element_type_to_string.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_stack_element_type_to_string_out.txt[]
----
...
[source,cpp]
----
include::../examples/api/types/script_engine_stack_element_value_to_string.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_stack_element_value_to_string_out.txt[]
----
...
- _C_ : The value to be converted as the one of the following,

include::api_to_stack_element_return.adoc[]

[.underline]#Parameters# 

- ___value_: Value to be converted from.
...
[source,cpp]
----
include::../examples/api/types/script_engine_to_stack_element.cpp[]
----
...
[source,txt]
----
include::../examples/api/types/script_engine_to_stack_element_out.txt[]
----
