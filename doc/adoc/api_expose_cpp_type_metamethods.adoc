...
Next, each function _NumberZs_addassign_ is registered as member metamethod __addassign_ through _ScriptEngine::registerMemberFunction_,
...
- __script_engine_ : ScriptEngine instance
- __this_ : The current instance
- __value_ : A value to be set. Its type can be a pointer of one defined in  xref:api_types.adoc#\_types[primitive types] or a pointer of other xref:api_expose_cpp_types.adoc#\_register_a_type[registered type].
...
The following code defines two functions performs an assignment from xref:language_data_types.adoc#\_float[Float] value or _Number_ object respectively,
