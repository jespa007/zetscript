...
To bind a ZetScript function that returns an instance of xref:language_data_types.adoc#_class[Class] type object it can specify __zetscript::ObjectScriptObject *__ or __zetscript::ClassScriptObject *__ return type on its function signature.
...
To bind a ZetScript function that returns an instance of xref:chapter_3_the_api.adoc#_registered_type[registered type] object it must specify __zetscript::ClassScriptObject *__ return type on its function signature. After returning  __zetscript::ClassScriptObject *__, it can fetch its C++ registered type by calling _ClassObject::to<_T *>_, where _T is the C&#43;&#43; type.
...
To bind a ZetScript function that accepts a <<language_data_types.adoc#_integer,Integer>> it must specify __zetscript::zs_int__ or _zs_int &#42;_ as parameter type on its function signature.
