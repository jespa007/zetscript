...
===== ZS_EVAL_OPTION_NO_EXECUTE

In order to eval script but no execute add _ZS_EVAL_OPTION_NO_EXECUTE_ option
...
In order to show byte code for a script add _ZS_EVAL_OPTION_PRINT_BYTE_CODE_ option

[source,cpp]
----
include::../examples/api/eval/eval_file_option_print_byte_code.cpp[]
----
...
[source,txt]
----
include::../examples/api/eval/eval_file_option_print_byte_code_out.txt[]
----

As we can see in the output _ZS_EVAL_OPTION_PRINT_BYTE_CODE_ option prints the byte code per each function. At the begin it prints summary,
