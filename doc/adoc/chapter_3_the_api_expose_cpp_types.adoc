...
Following the example of the _Number_ type and in order to compact the lines of code for the next examples of this section, it has been proposed put all registering and declaration of the functions in a file called `NumberZs.h`. Inside `NumberZs.h` there's a function called  _NumberZs_register_ that will register of those functions.
...
As parameters, the function must always include `ZetScript *` as the FIRST parameter,the current instance as the SECOND parameter and a maximum of 8 parameters as a pointer of xref:chapter_3_the_api_types#_built_in_types[built-in type] or a pointer of other registered type. 
...
- __arg1_ .. __arg8_: Arguments which types are pointers of xref:chapter_3_the_api_types#_built_in_types[built-in types] or other registered type.

After the constructor function is defined it has to be registered with `ZetScript::registerConstructor`. _registerConstructor_ it has the following signature,
...
As a return, the function it can return void, bool, zs_int,zs_float, zs_string, a pointer of xref:chapter_3_the_api_types#_built_in_types[built-in types] or a pointer of a registered type.
...
- _RegisteredType_: The registered type
- _ReturnType_: Return type can be void, bool, zs_int,zs_float, zs_string, a pointer of xref:chapter_3_the_api_types#_built_in_types[built-in types] or a pointer of a registered type.

[.underline]#Parameters# 

- __zs_ : ZetScript context
- __this_: The current instance
- __arg1_ .. __arg8_: Arguments which types are pointers of xref:chapter_3_the_api_types#_built_in_types[built-in types] or a pointer of a registered type.


After the member function is defined it has to be registered with `ZetScript::registerMemberFunction`. _registerMemberFunction_ it has the following signature,
...
As parameters, the function must always include `ZetScript *` as the FIRST parameter and a maximum of 9 parameters as a pointer of xref:chapter_3_the_api_types#_built_in_types[built-in type] or a pointer of other registered type. 

As a return, the function it can return void, bool, zs_int,zs_float, zs_string, a pointer of xref:chapter_3_the_api_types#_built_in_types[built-in type] or a pointer of other registered type.
...
- __zs_ : ZetScript context
- __arg1_ .. __arg9_: Arguments which types are pointers of xref:chapter_3_the_api_types#_built_in_types[built-in types] or a pointer of a registered type.


After the constructor function is defined it has to be registered with `ZetScript::registerStaticMemberFunction`. _registerStaticMemberFunction_ it has the following signature,
...
Inherited classes needs to know its base classes in order to register its parent variables and symbols already registered with the functions already seen in the section X.X.X and X.X.X respectively. To tell the which base class has an inherited class is done through extends with two parameters: The first parameter as the base type and the second parameter as inherited type.
...
ZetScript::extends<base_registered_type, registered_type>();
----

If for example we want to register MyCppTypeExtend and tell that is base of MyCppType Is done with the following snipped,
...
//---- BINDINGS
MyCppType * MyCppType_new(zetscript::ZetScript *_zs){
	return new MyCppType();
}

void MyCppType_constructor(zetscript::ZetScript *_zs, MyCppType *_this, zetscript::zs_int _data1){
	_this->data1=_data1;
}

zetscript::zs_int MyCppType_function1(zetscript::ZetScript *_zs, MyCppType *_this, zetscript::zs_int _arg){
	return _this->function1(_arg);
}

void MyCppType_delete(zetscript::ZetScript *_zs, MyCppType *_this){
	delete _this;
}
...
int main(){
	...
	// Register MyCppTypeExtend
	zs.registerType<MyCppTypeExtend>("MyCppTypeExtend");

	// MyCppTypeExtends extends from MyCppType
...
An important feature of ZetScript is that it supports c++ class inheritance for any in script class and the _this_ and _super_ keywords works as a normal behavior. For example, we could inherit MyCppTypeExtend as it shows in the following code,
