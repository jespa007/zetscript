...
To register a type is done by ScriptEngine::registerType. 
...
The following code defines two functions that accepts a xref:language_data_types.adoc#\_float[Float] value or _Number_ pointer type respectively,
...
The following code defines a function that returns a xref:language_data_types.adoc#\_integer[Integer],
...
To register a type and tell that is base of other type is done through xref:api_data_types.adoc#\_scriptengineextends[ScriptEngine::extends].
