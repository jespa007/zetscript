...
=== Call C function from script side

To call a C function from script side it must meet the following signature,
...
After the C function is created, it has to be registered by calling `ZetScript::registerFunction` passing the name of function and its reference.
...
This section will explain all cases of register a C function that returns the following script types,
