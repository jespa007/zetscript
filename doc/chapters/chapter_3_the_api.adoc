...
A native function to register must meet the following signature,

- *Return* :It can returns `void` (it doesn't returns any parameter) or some type from <<Built-in types>> or other registered type.
- *Parameters* : It includes `zetscript::ZetScript *` as first parameter and a maximum of 9 parameters as one defined in  <<Built-in types>> or other registered type.

*Syntax*

[source,c++]
----
ReturnType c_function_to_register(zetscript::ZetScript *_zs, Type1 _arg1, Type2 _arg2, ..., Type9 _arg9 ){
	....
}
----

After function is created it has to register by calling `ZetScript::registerFunction` passing a name that will be exposed as symbol name in the script side for its call and the native function reference.

For example,

 ZetScript zs;
 zs.registerFunction("script_symbol_name",c_function_to_register)


*Example*

The following example it register wrapper function called "sayHelloWorld" without arguments and without any return.
...
This section will explain how to register function that return the following types in the script side,
...
To register a function that returns a _Boolean_ value on the script side, the function must define the type _bool_ as the return type.
