...
=== Calling native function from script side

This section will explain the how to define and bind functions to call native function from script side.

==== Bind native function

To bind native function to be used as interface for script it has to be well defined. The definition of native function is a kind of C function that always include `zetscript::ZetScript *` as first parameter that ZetScript will set automatically in its calling. After defining `zetscript::ZetScript *` it can define until 9 parameters as <<built-in types>> or custom types defined by the user.

*Syntax*

[source,c++]
----
ReturnType function_interface(zetscript::ZetScript *_zs, Type1 _arg1, Type2 _arg2, ..., Type8 _arg8, Type8 _arg9 ){
	....
}
----

Finally, after having the native function well defined, bind the function is done by calling `ZetScript::bindFunction` by passing the name of function and the native function

[source,c++]
----
ZetScript::zetscript zs;
zs.bindFunction("function_interface",function_interface);
----
...
==== Defining function with a return type

A native function interface can define `void` to specify the returning of nothing, else the possible return types are the following,
...
Defining _bool_ type return value it defines a function that returns a boolean value
...
Defining _zs_int_ type return value it defines a function that returns a integer value
...
Defining _zs_float_ type return value it defines a function that returns a float value
...
Defining _const char *_ type return value it defines a function that returns a string as _const char *_ value
...
Defining _StringScriptObject *_ type return value it defines a function that returns string as a pointer of _String_ script object. The creation of _StringScriptObject_ it's done with usign _ZetScript_ context.
...
Defining _ArrayScriptObject *_ type return value it defines a function that returns a pointer of _Array_ script object. The creation of _ArrayScriptObject_ it's done through _ZetScript::newStringScriptObject_ usign _ZetScript_ context.
...
Defining _ObjectScriptObject *_ type return value it defines a function that returns a pointer of _Object_ script object. The creation of _ObjectScriptObject_ it's done through _ZetScript::newArrayScriptObject_ usign _ZetScript_ context.
...
A native function defined as script interface always has to define a `zetscript::ZetScript *` as first parameter then it can define until 9 parameters as <<built-in types>> or custom types defined by the user. Possible types as arguments are the following,
