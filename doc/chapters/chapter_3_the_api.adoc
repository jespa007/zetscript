...
For a native function to be called from _ZetScript_ it must follow apropiate signature to get its arguments and return its results and finally register that function.
As parameters, the native function to be exposed in _ZetScript_ must always include `zetscript::ZetScript *` type as first parameter and then a maximum of 9 parameters as types in one defined in  <<Built-in types>> or other registered type. As return, the native function to be exposed in _ZetScript_ it can returns `void` (it doesn't returns any parameter) or some type from <<Built-in types>> or other registered type.
...
- _ReturnType_: It can be `void` or some type from <<Built-in types>> or other registered type.
- _ParamType_: It can be one defined in  <<Built-in types>> or other registered type.


To register the native function its done by `ZetScript::registerFunction` passing a name as the symbol_name in _ZetScript_ and the native function reference.
...
This section will explain how to register functions to return the following ZetScript types:
...
To register a function that returns a _Boolean_ value in ZetScript it must create native function as _bool_ as the return type.
