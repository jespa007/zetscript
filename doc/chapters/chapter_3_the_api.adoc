...
To call a C function that returns a registered class (see section <<Register class>> to know more about how to register a class), it must create and register a C function that returns _<<_ClassScriptObject>>*_ as return type. To instantiate _ClassScriptObject_ is done through _<<ZetScript::newClassScriptObject>>_ using _ZetScript *_ context passing instantiated registered class by parameter.
