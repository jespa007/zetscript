...
After having the wrapper function defined, it can be binded by calling `ZetScript::registerFunction` passing the name of function and the wrapper function reference.

[source,c++]
----
ZetScript::zetscript zs;
zs.registerFunction("wrapper_function",wrapper_function);
----
...
A native function that returns a custom type value must define return type as _ClassScriptObject *_. The instantiation and setup of _ClassScriptObject_ it's done through these steps:
 ...
 . The custom type must to be binded through <<ZetScript::registerClass>>.
