...
ZetScript comes with the following set of built-in types to be used as interface beetwen C and ZetScript script functions. This section presents built-in types that will be used in the examples,
...
ZetScript instance usually used to give reference and perform operations in the current context. Due the big extension of the set of functions that ZetScript defines it will explain the most important (like <<ZetScript::eval>> already explained) along this document
...
See <<zs_string>> for more information
...
See <<StringScriptObject>> for more information
...
// push as integer value
object->push<zetscript::zs_int>(0)
----

See <<ArrayScriptObject>> for more information
...
// add field "a" as int
object->set<zetscript::zs_int>("a",0)
...
=== Calling C function from script side
...
ZetScript includes a set of types and helper functions. This document will explain the relevant ones,
