...
To expose a C function in the script side it has be defined as it follows,

- *Return* :It returns `void` (it doesn't returns any parameter) or some type from <<Built-in types>> or other registered type.
...
[source,c++]
----
ReturnType c_function_to_bind(zetscript::ZetScript *_zs, Type1 _arg1, Type2 _arg2, ..., Type9 _arg9 ){
	....
}
----

After defining the function, it has to registered by callind `ZetScript::registerFunction` passing the name of function and its reference.
...
This section explains how a C function to be binded in the script side, it should be defined in order to return the following script types,
...
To bind a function that returns a _Boolean_ in the script side, it has to define a C function that returns _bool_ type.
