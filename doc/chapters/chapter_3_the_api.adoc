...
The native function to bind is defined as C function and always includes `zetscript::ZetScript *` as first parameter and a maximum of 9 parameters as one defined in  <<Built-in types>> or custom types.
...
To create binding of a native function that returns a _boolean_ value to script side must define return type as _bool_.
