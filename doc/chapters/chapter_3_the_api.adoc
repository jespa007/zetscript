...
To bind native function to be used as interface for script it has to be well defined. The definition of native function is a C function that always include `zetscript::ZetScript *` as first parameter and a maximmum of 9 parameters more as <<built-in types>> or custom types defined by the user.
...
Defining _zs_string_ as type of return it defines a function that returns a string value
...
Defining _StringScriptObject_ as type of return it defines a function that returns a _String_ script object. The creation of _StringScriptObject_ it has be done with ZetScript context.
...
Defining _ArrayScriptObject_ as type of return it defines a function that returns a _Array_ script object. The creation of _ArrayScriptObject_ it has be done with ZetScript context.
...
Defining _ObjectScriptObject_ as type of return it defines a function that returns a _Object_ script object. The creation of _ObjectScriptObject_ it has be done with ZetScript context.
