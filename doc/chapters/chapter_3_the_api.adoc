...
At the c++ side, to loads and eval `file.zs` it's done through _evalFile_ function as it shows as it follows,
...
ZetScript has the following set of built-in types used beetwen function user and script functions,

- void : void type
- bool : C boolean type
- zs_int : int type as intptr_t
- zs_float : a float of 32 or 64 bits in function of compiler architecture 
- zs_string : string type
- StringScriptObject : String script object pointer
- ArrayScriptObject : Array script object pointer
- ObjectScriptObject : Object script object pointer
