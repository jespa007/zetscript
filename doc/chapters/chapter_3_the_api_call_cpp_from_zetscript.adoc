...
To register a C function that accepts a *xref:chapter_2_the_language_data_types.adoc#_boolean[Array]* it must specify *ArrayScriptObject &ast;* as parameter type on its function prototype.
...
To register a C function that accepts a *xref:chapter_2_the_language_data_types.adoc#_boolean[Object]* it must specify *ObjectScriptObject &ast;* as parameter type on its function prototype.
...
To register a C function that accepts a *xref:chapter_2_the_language_data_types.adoc#_boolean[Function]* it must specify *ScriptFunction &ast;* as parameter type on its function prototype. To make  _ScriptFunction_ callable from C++ it must to bind that using <<bindScriptFunction>>.
...
===== Parameter Registered Class

To register a C function that accepts a <<Registered class>> as a parameter, it must specify the *Registered class pointer* as parameter type on its function prototype.

*Example*

In the following example it registers the Point type and its properties `x` and `y`  (see <<Register class>> section to know about how to register class and properties) and it registers function `mul10Point` that accepts a Point type pointer as parameter. In the script execution it instances a Point type and is passed as argument to `mul10Point` that multiply its properties by 10.
