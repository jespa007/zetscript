...
Calling c++ code from _ZetScript_ is done by creating and registering a C function with a particular prototype. As parameters, the function must always include the type `zetscript::ZetScript *` as the first parameter and then a maximum of 9 parameters as types in one defined in <<Built-in Types>> or other registered types. As a return, the function may not return any parameter, a type defined in <<Built-in Types>> or another registered type.
...
After the function is created it has to register the function to be exposed in _ZetScript_. To register the function its done with `ZetScript::registerFunction` passing a name as the symbol_name in _ZetScript_ and function reference.
