...
To register a C function that returns a *xref:chapter_2_the_language_data_types.adoc#_boolean[Boolean]* type it must specify *bool* return type on its function prototype.
...
To register a C function that returns a <<_Integer_>> type it must specify *zs_int* return type on its function prototype.
...
To register a function that returns a _xref:chapter_2_the_language_data_types.adoc#_string[String]_ type, it can create and register a C function that returns _<<zs_string>>_ as return type.
