=== Calling c++ from ZetScript

To call c++ code from _ZetScript_ it must create and register a C function with an apropiate prototype to get its arguments and return its results.
...
After the function is created it has to register the function to be exposed in _ZetScript_. To register the function its done by `ZetScript::registerFunction` passing a name as the symbol_name in _ZetScript_ and function reference.
