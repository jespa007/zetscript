...
To call ZetScript from C++ is it done through `ZetScript::bindScriptFunction` that it makes a ZetScript function callable for C&plus;&plus;. `ZetScript::bindScriptFunction` is sets the  function signature and the name of symbol or ScriptFunction to be binded of.

*Syntax*

[source,c++]
----
// bind script function from function name
 ZetScript::bindScriptFunction<ReturnType(ParamType _arg1, ParamType _arg2, ..., ParamType _arg9 )>("name_script_function");

// bind script function from ScriptFunction 
 ZetScript::bindScriptFunction<ReturnType(ParamType _arg1, ParamType _arg2, ..., ParamType _arg9 )>(ScriptFunction * _script_function)
----

Where,

 - ReturnType: It can be void or some type defined in [ZetScript built-in types] or other registered type.
 - ParamType: It can be one defined in [ZetScript built-in types] or other registered type.

*Example*

The following example it binds hellow world function without no parameters and no return
...
To register a C function that returns a *xref:chapter_2_the_language_data_types.adoc#_boolean[String]* it can specify *const char &ast;* return type on its function signature.
...
To register a C function that returns a *xref:chapter_2_the_language_data_types.adoc#_boolean[String]* it can specify *zs_string* return type on its function signature.
...
To register a C function that returns a *xref:chapter_2_the_language_data_types.adoc#_boolean[String]* it can specify *zs_sStringScriptObject &ast;* return type on its function signature.
...
To bind a ZetScript function that returns a *xref:chapter_2_the_language_data_types.adoc#_boolean[Object]* it must specify *ObjectScriptObject &ast;* return type on its function signature.
...
To bind the ZetScript function that returns a <<registered class>>, you must specify a registered class pointer as return type in the function signature.
