...
To make a ZetScript function callable from C&plus;&plus; is done through `ZetScript::bindScriptFunction`. `ZetScript::bindScriptFunction` it binds ZetScript function by the C&plus;&plus; function signature and the symbol name or ScriptFunction object.
...
To bind a ZetScript function that returns an *xref:chapter_2_the_language_data_types.adoc#_script_class[instance of class type object]* it can specify *ObjectScriptObject &ast;* or *ClassScriptObject &ast;* return type on its function signature.
...
To bind a ZetScript function that returns an instance of *xref:chapter_3_the_api.adoc#_registerer_type[registered type]* object it must specify *ClassScriptObject &ast;* return type on its function signature. After returning  *ClassScriptObject &ast;*, it can fetch its C++ registered type by calling `ClassScriptObject::to<_T *>`, where _T is the C&plus;&plus; type.

After using the object returned by the function it has to decrease its reference count by calling `ZetScript::unrefLifeTimeObject`.

*Example*

In this example, it registers `Point` type, its constructor and its properties `x` and `y`. After evaluate ZetScript code that defines a function that return a new object type `Point`, it is binded to be callable in C++.  When it calls `returnPoint` in C&plus;&plus;, it returns a variable as type *ClassScriptObject &ast;* object that it prints its contents. Later it gets the registered class reference and also it prints its contents.
