...
Like we've seen in the returning type for registered class that creates *ClassScriptObject &ast;* and wraps the instatiated C++ type, here it does the same but in reverse mode. So to bind a ZetScript function that returns *xref:chapter_3_the_api.adoc#_registered_class[Registered Class]* type it must specify *ClassScriptObject &ast;* return type on its function signature. *ClassScriptObject &ast;* is 

After returning ZetScript class type object, it can fetch C++ registered type by applying "ClassScriptObject::to<_T>".
...
==== bind member function from instantiated global object

It can also call from instantiated object
