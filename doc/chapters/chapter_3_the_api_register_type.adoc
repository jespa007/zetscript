...
To register C&plus;&plus; class it's done by passing the C&plus;&plus; class or struct type with `ZetScript::registerType<_Type>(_script_name)`
...
- _Type: Struct/Class type type 
- _symbol_name: Symbol name linked to class name in ZetScript.
...
==== Register no instantiable class

Register not instantiable type it means that it cannot create new objects (i.e use *new* keyword).
To register type no instantible type is done simple by calling *ZetScript::registerClass* with type as template and the name that will be referenced in the script.

Wht it can be useful the using of non instantiable types ? For example to have control of instantiated types in the C++ program
