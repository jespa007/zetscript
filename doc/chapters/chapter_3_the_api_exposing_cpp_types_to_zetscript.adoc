...
Let's define a C/C&plus;&plus; type 'Number' type in a file 'Number.h',
...
Next, a function called 'NumberZs_register' is defined in the file 'NumberZs.h'. Inside this file everything related to the type 'Number' will be recorded. In this case, it starts by registering the 'Number' type to be exposed in ZetScript,

[source,cpp]
----
void NumberZs_register(ZetScript *_zs);
 	//register Number type
 	_zs->registerType<Number>("Number");
}
----
...
Now, let's supose we want to instantiate a 'Number' evaluating the following script code,

[source,cpp]
----
	// It throws an error that 'Number' is not instanciable
 	zs.eval(
