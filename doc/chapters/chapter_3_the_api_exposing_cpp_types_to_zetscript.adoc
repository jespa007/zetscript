...
zs_int NumberZs_toInteger(zetscript::ZetScript *_zs,Number *_this){
	ZS_UNUSUED_PARAM(_zs);
	return _this->value;
}
...
Next, it's registered as `toInteger` with `registerMemberFunction`,
...
- __this_ : instance registered object
