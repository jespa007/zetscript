...
#define INSTRUCTION_PROPERTY_FOR_STORE								(INSTRUCTION_PROPERTY_USE_PUSH_STK | INSTRUCTION_PROPERTY_CONTAINER_SLOT_ASSIGMENT)
...
#define INSTRUCTION_CONST_TO_STK_CONST_PROPERTY(properties)\
	 (properties) & INSTRUCTION_PROPERTY_ZS_INT ? ZS_STK_PROPERTY_INT \
	:(properties) & INSTRUCTION_PROPERTY_BOOL ? ZS_STK_PROPERTY_BOOL \
	:(properties) & INSTRUCTION_PROPERTY_ZS_FLOAT ? ZS_STK_PROPERTY_FLOAT \
	:(properties) & INSTRUCTION_PROPERTY_STRING ? (ZS_STK_PROPERTY_SCRIPT_OBJECT | ZS_STK_PROPERTY_READ_ONLY) \
	:0
...
#ifndef __arm__
// Instruction estructure is unaligned estructure due pragma(1) and in ARM environment it derives a
// seg fault bus error when converts from float from intptr_t so in ARM platform is disabled. Another
// option could be align the estructure puting value op2 at the begining of Instruction, to ensure the
