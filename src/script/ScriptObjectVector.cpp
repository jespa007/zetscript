...
namespace zetscript{
	...
	StackElement * ScriptObjectVector::getUserElementAt(unsigned _idx){
	...
	bool ScriptObjectVector::eraseUserElementAt( unsigned idx){//onst zs_string & varname){
	...
	void ScriptObjectVector::eraseAllUserElements(){
		for(unsigned i=0; i <stk_user_elements.count; i++){
