		...
		char * deserialize_value(
			...
			if (*str_current == '\"') {// try string ...
				...
				if (bytes_readed > 0) {
					...
					if((number_value=Float::parse(default_str_value)) != NULL){
						ZS_FLOAT_COPY(&stk_json_element->value,number_value);
						stk_json_element->properties=STACK_ELEMENT_PROPERTY_FLOAT;
						delete number_value;
					}
				}else{
					json_deserialize_error(data,str_start,line,"Cannot deduce json value. Json value can be Number,Boolean,String, Array or ScriptObject");
					return NULL;
				}
			}
			return str_current;
		...
		char * deserialize_object(JsonDeserializeData *data, const char * str_start, int & line,StackElement *stk_json_element) {
			...
			if(*str_current != '{'){
				json_deserialize_error(data
						...
						,stk_json_element!=NULL?data->script_engine->stackElementTypeToString(stk_json_element).toConstChar():"");
				return NULL;
			}
