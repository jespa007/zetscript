...
namespace zetscript{
	...
	String StringUtils::intToString(zs_int _number, const String & _format){
		...
		if(!_format.empty()){
			String sf=StringUtils::toLower(_format);
	...
	bool StringUtils::isHexaDigit(char c){
		return ((('0' <= c) && (c<='9')) || ('a'<=(toLower(c))&&(toLower(c)<='f')));
	}

	char *StringUtils::advanceDigits(char *aux_p){

		while(isDigit(*aux_p))	aux_p++;
		return aux_p;
	}

	char *StringUtils::advanceHexaDigits(char *aux_p){
