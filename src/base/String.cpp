...
namespace zetscript{
	...
	bool String::isHexaDigit(char c){
		return ((('0' <= c) && (c<='9')) || ('a'<=(Character::toLower(c))&&(Character::toLower(c)<='f')));
	}

	char *String::advanceDigits(char *aux_p){

		while(isDigit(*aux_p))	aux_p++;
		return aux_p;
	}

	char *String::advanceHexaDigits(char *aux_p){

		while(isHexaDigit(*aux_p))	aux_p++;
		return aux_p;
	}

	String::NumberType String::isNumber(const String & test_str_number){

		if(test_str_number.isEmpty()){
			return String::NumberType::NUMBER_TYPE_INVALID;
		}
		...
		if(Character::toLower(*str)=='b'){
			return NumberType::NUMBER_TYPE_BINARY;
		}
