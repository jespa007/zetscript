...
namespace zetscript{
		...
		while(v>=0){
			...
			(scope->properties & SCOPE_PROPERTY_UNUSUED)){
				...
				if(scope->scope_parent != NULL){
					...
					std::vector<Scope *> *childs=scope->scope_parent->scopes;
					for(unsigned i=0; i < childs->size(); i++){
						Scope *child=(Scope *)childs->at(i);
						if(child==scope){
	...
	void ScopeFactory::clear(){
		...
		for(
			...
			Scope * info_scope = (Scope *)scopes->at(v);//(Scope *)scopes->get(v);
