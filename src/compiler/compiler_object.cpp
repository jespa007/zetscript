...
namespace zetscript{
	...
	char * compiler_object_object(CompilerData *compiler_data,const char *s,int & line,  ScriptScope *scope_info, Vector<CompilerInstruction *>	*	compiler_instructions){
		...
		while (*aux_p != '}' && *aux_p != 0){
			...
			if((stk_key_object = compiler_data->zs->getStkConstantStringScriptObject(str_key))==NULL){
				stk_key_object=compiler_data->zs->registerStkConstantStringScriptObject(str_key,symbol_value);
			 }
