...
namespace zetscript{
	...
	char * eval_keyword_if_else(EvalData *eval_data,const char *s,int & line,  Scope *scope_info){
		...
		if(key_w == Keyword::KEYWORD_IF){
			do{
				...
				eval_data->current_function->eval_instructions.push(ei_aux=new EvalInstruction(BYTE_CODE_JNT));
				ei_aux->instruction_source_info.file=eval_data->current_parsing_file;
				ei_aux->instruction_source_info.line=line;

				ei_if_jnt=ei_aux;
				int idx_start_block=eval_data->current_function->eval_instructions.length();
				...
				int idx_end_block=eval_data->current_function->eval_instructions.length();
				...
				if(else_key){ // it finish if-else
					...
					eval_data->current_function->eval_instructions.push(ei_aux=new EvalInstruction(
							BYTE_CODE_JMP
							, INSTRUCTION_VALUE_OP1_NOT_DEFINED
							,eval_data->current_function->eval_instructions.length()
							));
			...
			for(int i=0; i < ei_else_end_jmp.length(); i++){
				...
				ins->value_op2=eval_data->current_function->eval_instructions.length()-ins->value_op2;
			}
			return aux_p;
		}
		return NULL;
	...
	char * eval_keyword_switch(EvalData *eval_data,const char *s,int & line,  Scope *scope_info){
		...
		if(key_w == Keyword::KEYWORD_SWITCH){
			int idx_start_instruction = (int)(eval_data->current_function->eval_instructions.length());;
			int size_ei_cases=0;
			...
			while(*aux_p != '}' && *aux_p != 0){
				...
				if(key_w == KEYWORD_CASE){
					...
					eic_cases.push(eval_instruction_case);
					...
					eval_instruction_case->ei_je_instruction=new EvalInstruction(
							BYTE_CODE_JE_CASE
							, INSTRUCTION_VALUE_OP1_NOT_DEFINED
							,((int)(eval_data->current_function->eval_instructions.length()))-idx_start_instruction	 // offset
					);
					...
					// update size cases
					size_ei_cases+=(eval_instruction_case->ei_load_symbols.length()+1); // +1 is for je (i.e je_instruction)
					...
					ei_jmp_default=new EvalInstruction(
							BYTE_CODE_JMP_CASE
							, INSTRUCTION_VALUE_OP1_NOT_DEFINED
							,((int)(eval_data->current_function->eval_instructions.length()))-idx_start_instruction+1	 // offset
				...
				if((eval_is_keyword(aux_p) == Keyword::KEYWORD_BREAK)){ // it cuts current expression to link breaks...
					...
					if(*aux_p != '}'){ // not end insert jmp
						...
						eval_data->current_function->eval_instructions.push(ei_break_jmp);
						ei_break_jmps.push(ei_break_jmp);
					}


				}else if(is_default){
					//EvalInstruction *ie_last=(EvalInstruction *)eval_data->current_function->eval_instructions.items[eval_data->current_function->eval_instructions.length()-1];
					EVAL_ERROR_FILE_LINE_GOTOF(
						eval_data->current_parsing_file
						,line
						,eval_keyword_switch_error
			...
			// end instruction
			int offset_end_instruction=((int)(eval_data->current_function->eval_instructions.length()))-idx_start_instruction;
			...
			if(offset_end_instruction > 0 && eic_cases.length() > 0){ // there's conditions to manage
				...
				offset_end_instruction=eval_data->current_function->eval_instructions.length();
				for(int i=idx_start_instruction; i < eval_data->current_function->eval_instructions.length();i++){
