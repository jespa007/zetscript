		 ...
		 if((aux=parse_literal_number(
			...
			if(*aux=='\"'){
				...
				if(default_str_value=="undefined"){ // undefined literal
					byte_code=ByteCode::ByteCodeId::BYTE_CODE_ID_LOAD_UNDEFINED;
				}else if(default_str_value=="null"){ // null literal
						byte_code=ByteCode::ByteCodeId::BYTE_CODE_ID_LOAD_NULL;
					...
					if(default_str_value == SYMBOL_VALUE_THIS || default_str_value == SYMBOL_VALUE_SUPER){
						...
						if(default_str_value == SYMBOL_VALUE_SUPER){
							...
							byte_code= ByteCode::ByteCodeId::BYTE_CODE_ID_LOAD_THIS;// ZS_INSTRUCTION_PROPERTY_ACCESS_TYPE_THIS;
						}
					}else{
						...
						if((local_symbol=eval_find_local_symbol(eval_data,scope_info,default_str_value)) != NULL){ // local sy
							...
							if(local_symbol->properties & ZS_SYMBOL_PROPERTY_FUNCTION){
								byte_code= ByteCode::ByteCodeId::BYTE_CODE_ID_LOAD_FUNCTION;
								value=(zs_int)local_symbol;

							}else if(local_symbol->properties & ZS_SYMBOL_PROPERTY_TYPE){
								byte_code= ByteCode::ByteCodeId::BYTE_CODE_ID_LOAD_TYPE;
								value=-1;
							}else{
