...
namespace zetscript{
	...
	char * eval_keyword_static(EvalData *eval_data,const char *s,int & line,  ScriptScope *scope_info){
		...
		if(eval_is_keyword(aux_p)==Keyword::KEYWORD_STATIC){
			...
			if(key_w == Keyword::KEYWORD_FUNCTION){
				...
				EVAL_ERROR_FILE_LINEF(eval_data->current_parsing_file,line,"expected 'var' or 'function' after 'static'");
			}
		}
	...
	char *eval_keyword(EvalData *eval_data,const char *s, int & line, ScriptScope *scope_info, Keyword keyw){
		// check if condition...
