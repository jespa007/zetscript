...
namespace zetscript{
	...
	bool 								ArrayScriptObject::elementInstanceOf(int _pos){
		...
		return this->se->stackElementInstanceOf<_T>(stk_elements.get(_pos));
	}
	...
	_T	ArrayScriptObject::get(int _pos){
		...
		return this->se->stackElementTo<_T>(stk_elements.get(_pos));
	}
	...
	void	ArrayScriptObject::set(int _pos, _T _element){
		...
		auto stk=this->se->toStackElement(_element);
