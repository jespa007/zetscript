...
/*	typedef struct{
	...
	// Helpers
	//
	StringObject * StringObject::newStringObject(ZetScript *_zs, const String & _str){
		StringObject *so=new StringObject(_zs);
		so->set(_str);

		return so;
	}

	StringObject *StringObject::newStringObjectAddStk(ZetScript *_zs,StackElement *stk_result_op1,StackElement *stk_result_op2){
		StringObject *so_ref=NULL;
		// we have to create an new string variable
		if(STACK_ELEMENT_IS_STRING_OBJECT(stk_result_op1)){
			so_ref=(StringObject *)stk_result_op1->value;
		}

		if(so_ref == NULL){
		   if(STACK_ELEMENT_IS_STRING_OBJECT(stk_result_op2)){
			   so_ref=(StringObject *)stk_result_op2->value;
		   }else{
		...
		for(int i=0; i < 2; i++){
			...
			if(stk_src_item->properties & STACK_ELEMENT_PROPERTY_OBJECT){
				*(*str_dst_it)=((Object *)stk_src_item->value)->toString();
			}else{
	...
	StringObject * StringObject::format(
			ZetScript *_zs,
			StackElement *_stk
		...
		if(_stk_args->properties & STACK_ELEMENT_PROPERTY_OBJECT){
			Object *so=(Object *)_stk_args->value;
			if(so->type_id == TYPE_ID_OBJECT_ARRAY){
				sov=(ArrayObject *)so;
			}
		}
						...
						if((ptr_idx_num=String::parseInt(str_num_aux))!=NULL){
							...
							if(idx_num >=0 && idx_num<(int)sov->length()){ // print
								...
								if(stk_arg.properties & STACK_ELEMENT_PROPERTY_OBJECT){
									str_format_results=((Object *)stk_arg.value)->toString();
								}else{
	...
	StringObject::StringObject(
			ZetScript *_zs
			,const String & _str
	):Object(_zs,TYPE_ID_OBJECT_STRING){
	...
	const char *StringObject::getConstChar(){
