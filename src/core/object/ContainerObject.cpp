...
namespace zetscript{
	...
	ContainerObject::ContainerObject(
			ZetScript *_zs
			,TypeId _ref_type_id
	):Object(_zs, _ref_type_id){
		container_slots=new List<ContainerSlot *>;
	}

	List<ContainerSlot *>	*ContainerObject::getListContainerSlotsRef(){
		return container_slots;
	}

	void ContainerObject::addContainerSlot(ContainerSlot *_container_slot){
		_container_slot->setSrcContainerRef(this);
	}


	void ContainerObject::removeContainerSlot(ContainerSlot *_container_slot, VM_ScopeBlock 	*	_scope_block){
		if(_container_slot!=NULL){
			...
			vm_unref_shared_object(vm,_container_slot->getSrcContainerRef(),_scope_block );

		}
	}

	void ContainerObject::onDettachContainerSlotNode(ListNode<ContainerSlot *> *_current_node){
		_current_node->data->setSrcContainerRef(NULL);
	}

	void ContainerObject::removeAllSlots(){
	...
	void ContainerObject::printReferences(){
