...
namespace zetscript{
	...
	bool 								DictionaryScriptObject::elementInstanceOf(
		const String & _key
	){
		StackElement *stk;
	...
	template<typename _T>
	_T	DictionaryScriptObject::get(const String & _key){
		StackElement *stk;
		if((stk=getStackElementByKeyName(_key))==NULL){
			ZS_THROW_EXCEPTION("key '%s' not exist",_key.toConstChar());
		}
	...
	template<typename _T>
	void	DictionaryScriptObject::set(const String & _key_name, _T _value){
		StackElement stk=this->zs->toStackElement<_T>(_value);

		// Create shared object due 'DictionaryScriptObject::set' is called from user program
