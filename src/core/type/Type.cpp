...
namespace zetscript{
	...
	bool Type::isNativeType(){

		 return ((properties & ZS_TYPE_PROPERTY_C_OBJECT_REF) != 0);
	}

	bool Type::isNonInstantiable(){
		 return ((properties & ZS_TYPE_PROPERTY_NON_INSTANTIABLE) != 0);
	}
	//------------------------------------------------------------

	Type::Type(ZetScript *_zs
			,TypeId _id
		...
		base_type_ids=new Vector<TypeId>;
	...
	bool Type::extendsFrom(TypeId _type_id){
		if(_type_id==this->id){
			return true;
		}
	...
	Type * 				Type::getType(TypeId _type_id){
		return type_factory->getType(_type_id);
	}

	TypeId				Type::getTypeIdFromTypeNamePtr(const char * s){
		return type_factory->getTypeIdFromTypeNamePtr(s);
	}
	...
	// MEMBER VARIABLES
	//
	Symbol				* 	Type::registerMemberVariable(
	...
	// MEMBER PROPERTIES
	//
	Symbol *Type::registerMemberProperty(
	...
	Symbol				* 	Type::registerMemberPropertyMetamethod(
	...
	Symbol * Type::registerMemberFunction(
		...
		if((_function_properties & FUNCTION_PROPERTY_C_OBJECT_REF)==0){ // we only allow repeated symbols on native functions...
			...
			//if(getSymbol(_function_name,(int8_t)_params_len,false) != NULL){
				...
				if((existing_symbol=getSymbol(_function_name.toConstChar(), ZS_NO_PARAMS_SYMBOL_ONLY)) != NULL){
					...
					Function *existing_sf=(Function *)existing_symbol->ref_ptr;
			...
			Function::checkNativeFunctionParams(this->scope,_return_type_id,_function_name,*_params,_params_len);
		}
		...
		Function *sf_current=(Function *)symbol_function->ref_ptr;
					...
					if((_function_properties & FUNCTION_PROPERTY_C_OBJECT_REF)){ // if-native
						...
						if(_function_properties & FUNCTION_PROPERTY_STATIC){
							...
							case METAMETHOD_SHR: // >> shift right
								if(_return_type_id != TYPE_ID_OBJECT_CLASS){
									ZS_THROW_RUNTIME_ERROR("Error registering static metamethod '%s::%s'. Expected return 'ClassObject *' but it was '%s'",
										this->name.toConstChar(),
										_function_name.toConstChar(),
										Rtti::demangle(this->type_factory->getType(_return_type_id)->native_name.toConstChar()).toConstChar()
									);
								...
								/*if(this->type_factory->getType(_return_type_id)->native_name != this->native_name){
							...
							case METAMETHOD_IN:
								...
								if(_return_type_id != TYPE_ID_BOOL_C){
							...
							case METAMETHOD_BWC:
								...
								if(_return_type_id != TYPE_ID_OBJECT_CLASS){
									ZS_THROW_RUNTIME_ERROR("Error registering member metamethod '%s::%s'. Expected return 'ClassObject *' but it was '%s'",
										this->name.toConstChar(),
										_function_name.toConstChar(),
										Rtti::demangle(this->type_factory->getType(_return_type_id)->native_name.toConstChar()).toConstChar()
									);
							...
							case METAMETHOD_POST_DEC: // i--
								if(_return_type_id != TYPE_ID_OBJECT_CLASS /*&& _return_type_id != TypeId::TYPE_ID_VOID_C*/){
									ZS_THROW_RUNTIME_ERROR("Error registering member metamethod '%s::%s'. Expected return 'ClassObject *' but it was '%s'",
										this->name.toConstChar(),
										_function_name.toConstChar(),
										Rtti::demangle(this->type_factory->getType(_return_type_id)->native_name.toConstChar()).toConstChar()
	...
	//---------------------------------------------------------
	Symbol *    Type::getSymbolVariableMember(const char  *_const_symbol_name, bool include_inherited_symbols){
	...
	Symbol *    Type::getSymbolMemberFunction(const char * _const_symbol_name, int8_t n_params, bool include_inherited_symbols){
		...
		while(i >= idx_end){
			...
			if(strcmp(member_symbol->name.toConstChar(),_const_symbol_name)==0){
				Function *sf=(Function *)member_symbol->ref_ptr;
	...
	Symbol				* 	Type::getSymbol(const char  * _const_symbol_name, int8_t n_params,bool include_inherited_symbols){
