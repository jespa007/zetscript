...
namespace zetscript{
		Instruction::Instruction(){
			byte_code=BYTE_CODE_INVALID;
			value_op1= INSTRUCTION_VALUE_OP1_NOT_DEFINED;
		...
		zs_float Instruction::getConstantFloat(){
			...
			if(((this->byte_code == BYTE_CODE_LOAD_FLOAT) || (this->properties & INSTRUCTION_PROPERTY_FLOAT))==false){
		...
		zs_int Instruction::getConstantInt(){
			...
			if(((this->byte_code == BYTE_CODE_LOAD_INT) || (this->properties & INSTRUCTION_PROPERTY_INT))==false){
		...
		String Instruction::getConstantValueOp2ToString( bool _str_with_quotes){
			...
			if(byte_code ==BYTE_CODE_LOAD_BOOL || (this->properties & INSTRUCTION_PROPERTY_BOOL) ){
				...
				value_op2_string=Float::toString(this->getConstantFloat());
		...
		int instruction_num_required_stack(Instruction *_instruction){
			...
			if(properties & INSTRUCTION_PROPERTY_ILOAD){
				...
				case BYTE_CODE_LOAD_OBJECT_ITEM:
					...
					if((_instruction->properties & INSTRUCTION_PROPERTY_CALLING_FUNCTION)!=0){
						return 1;
					}
