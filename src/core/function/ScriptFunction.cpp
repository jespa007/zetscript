...
#define ZS_GET_ILOAD_R_STR(properties,value) \
	((properties) & INSTRUCTION_PROPERTY_ILOAD_R_ACCESS_THIS_VAR) ? ((Symbol *)_script_type->scope->symbol_variables->get(value))->name.toConstChar()\
...
namespace zetscript{
	...
	InstructionSourceInfo * ScriptFunction::getInstructionInfo(Instruction *instruction){
	 ...
	 void ScriptFunction::printGeneratedCode(ScriptFunction *sfo,ScriptType *_script_type){

		// PRE: it should printed after compile and updateReferences.
		// first print functions  ...
		zetscript::ScriptEngine *script_engine=sfo->script_engine;
		int sum_stk_load_stk=0;
		int max_acc_stk_load=0;
		int req_stk;


		if(sfo->properties & SCRIPT_FUNCTION_PROPERTY_NATIVE_OBJECT_REF){ // c functions has no script instructions
			return;
		}
		...
		if(_script_type==NULL){ // no type is a function on a global scope
			symbol_ref=sfo->name;
		}else{ // is a type
			symbol_ref=sfo->name;//+String("::")+String("????");
			class_str=String(_script_type->name)+"::";
		}



		printf("______________________________________________________________\n\n");
		printf(" ScriptFunction: '%s%s'                                             \n",class_str.toConstChar(),symbol_ref.toConstChar());
		printf(" Stack code: %i                                       		  \n",sfo->min_code_stack_needed);
		printf(" Stack local vars: %i                                         \n",sfo->local_variables->length());
		   ...
		   case INSTRUCTION_PROPERTY_ILOAD_RR: /* perfom Register-Register*/ \
			...
			case  BYTE_CODE_NEW_OBJECT_BY_TYPE:
				printf("[" ZS_HEADER_FORMAT_INSTRUCTION "]\t%s\t%s\n"
					...
					,(int8_t)instruction->value_op1!=ZS_UNDEFINED_IDX?ZS_GET_TYPE_NAME(sfo->script_types_factory,instruction->value_op1):"???"
				);
				break;
	...
	void ScriptFunction::checkNativeFunctionParams(ScriptScope *_scope,int _idx_return_type,const String & _function_name,FunctionParam *_params,int _params_len){

		ZS_UNUSUED_PARAM(_idx_return_type);

		for(int i=0; i < _scope->symbol_functions->length();i++){
	...
	ScriptFunction	*	ScriptFunction::getInstructionFunctionLastCall(Instruction * _instruction){
	...
	void				ScriptFunction::clearFunctionLastCalls(){
		this->instruction_last_native_call.clear();
	}

	void				ScriptFunction::setInstructionFunctionLastCall(Instruction * _instruction,ScriptFunction *_sf){
		short idx= (_instruction-this->instructions);///sizeof(Instruction *);
		this->instruction_last_native_call.set(idx,(zs_int)_sf);
	}

	Symbol * ScriptFunction::registerLocalArgument(
			 ScriptScope * _scope_block
			, const char *_file
			, short _line
		...
		Symbol * symbol=NULL;
		short idx_position_aux=(short)local_variables->length();

		if((symbol=_scope_block->registerSymbolVariable(
				_file
				,_line
				, _symbol_name
				, SCRIPT_SCOPE_REGISTER_PROPERTY_CHECK_REPEATED_SYMBOLS_CURRENT_LEVEL
		))==NULL){
				return NULL;
		}
	...
	Symbol * ScriptFunction::registerLocalFunction(
		...
		if(symbol_repeat != NULL){ // symbol found
			...
			(sf_repeat->properties & SCRIPT_FUNCTION_PROPERTY_NATIVE_OBJECT_REF) != (_function_properties & SCRIPT_FUNCTION_PROPERTY_NATIVE_OBJECT_REF)
							||
			// ... or both are script but not in the same scope
			((((sf_repeat->properties | _function_properties) & SCRIPT_FUNCTION_PROPERTY_NATIVE_OBJECT_REF) == 0) && 	(symbol_repeat->scope != _scope_block) )
			...
			if(((sf_repeat->properties | _function_properties) & SCRIPT_FUNCTION_PROPERTY_NATIVE_OBJECT_REF) == 0){
			...
			((ScriptFunction *)symbol_repeat->ref_ptr)->properties|=SCRIPT_FUNCTION_PROPERTY_DEDUCE_AT_RUNTIME; //
		}
		...
		if(((_function_properties & SCRIPT_FUNCTION_PROPERTY_NATIVE_OBJECT_REF) != 0) && (symbol_repeat!=NULL)){
			((ScriptFunction *)symbol->ref_ptr)->properties|=SCRIPT_FUNCTION_PROPERTY_DEDUCE_AT_RUNTIME;
		}
	...
	void ScriptFunction::updateParams(
		...
		if(_params_len>=ZS_SCRIPT_FUNCTION_PARAMETER_COUNT_MAX){
			ZS_THROW_EXCEPTION("Reached max parameter count (count:%i max:%i)"
				,_params_len
				,ZS_SCRIPT_FUNCTION_PARAMETER_COUNT_MAX
			);
		}
	...
	bool ScriptFunction::linkUnresolvedSymbols(){
		...
		if(unresolved_symbols.length() > 0){

			const char *str_aux=NULL;
			int i=0;
			while(i < unresolved_symbols.length()){
				...
				if((idx_sc_found= script_types_factory->getScriptTypeId(ptr_str_symbol_to_find))!= ZS_UNDEFINED_IDX){ // check if type
					...
					if(unresolved_instruction->byte_code==BYTE_CODE_THIS_CALL){
						...
						ScriptType *sc_sf=this->scope->getOwnerType();
						for(int j = 0; j < sc_sf->scope->symbol_functions->length(); j++){
						...
						// global scope
						symbol_found = ZS_MAIN_SCOPE(this)->getSymbol(ptr_str_symbol_to_find,ZS_NO_PARAMS_SYMBOL_ONLY,SCRIPT_SCOPE_REGISTER_PROPERTY_CHECK_REPEATED_SYMBOLS_DOWN);
					}
				}
		...
		return unresolved_symbols.length()==0;
	}
	...
	zetscript::ScriptEngine *ScriptFunction::getScriptEngine(){
	...
	ScriptFunction::~ScriptFunction(){
		...
		for(int i=0; i < instruction_source_infos.length(); i++){
