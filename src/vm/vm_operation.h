...
#define VM_OPERATION_COMPARE(__C_OP__, __METAMETHOD_BYTE_CODE_OPERATION__,__IS_JE_CASE__)\
	...
	default:\
		if(STK_VALUE_IS_TYPE(stk_result_op1)){\
			...
			if(__METAMETHOD_BYTE_CODE_OPERATION__ ==METAMETHOD_BYTE_CODE_EQU || __METAMETHOD_BYTE_CODE_OPERATION__ ==METAMETHOD_BYTE_CODE_NEQU){\
				PRAGMA_POP\
				...
				}else if((stk_result_op1->value == IDX_TYPE_UNDEFINED) && (stk_result_op1->properties != IDX_TYPE_UNDEFINED) && (__METAMETHOD_BYTE_CODE_OPERATION__ ==METAMETHOD_BYTE_CODE_NEQU)){\
					VM_PUSH_STK_BOOLEAN(true);\
				}else{\
					VM_PUSH_STK_BOOLEAN(stk_result_op1->value __C_OP__ stk_result_op2->value);\
				}\
