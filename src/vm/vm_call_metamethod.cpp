		...
		if(_metamethod == METAMETHOD_ADD_ASSIGN){
			if(	STACK_ELEMENT_IS_STRING_SCRIPT_OBJECT(stk_result_op1)){\
				(((StringScriptObject *)stk_result_op1->value)->str_ptr)->append(\
						(stk_result_op2->properties & STACK_ELEMENT_PROPERTY_OBJECT)?(((ScriptObject *)stk_result_op2->value)->toString()):data->script_engine->stackElementValueToString(ZS_VM_STR_AUX_PARAM_0,ZS_VM_STR_AUX_MAX_LENGTH,stk_result_op2)\
				);\
