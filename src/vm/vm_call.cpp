				...
				if((sf_call_stk_function_ref->properties & (STACK_ELEMENT_PROPERTY_FUNCTION))==0){
					...
					if(instruction->byte_code==BYTE_CODE_INDIRECT_THIS_CALL){
						...
						ZS_VM_STOP_EXECUTE("Cannot call 'this.%s' as type '%s'. 'this.%s' is not function"
								,SFI_GET_SYMBOL_NAME(_script_function,instruction)
								,data->script_engine->stackElementTypeToString(sf_call_stk_function_ref).toConstChar()
								,SFI_GET_SYMBOL_NAME(_script_function,instruction)
						);
						...
						ZS_VM_STOP_EXECUTE("Error trying to call a function from stack. StackElement value is '%s' as type '%s'"
								,data->script_engine->stackElementValueToString(sf_call_stk_function_ref).toConstChar()
								,data->script_engine->stackElementTypeToString(sf_call_stk_function_ref).toConstChar()
								,SFI_GET_SYMBOL_NAME(_script_function,instruction)
						);
					}
			...
			if((sf_call_script_function->properties & SCRIPT_FUNCTION_PROPERTY_NATIVE_OBJECT_REF) == 0){ // if script function...
				...
				if(sf_call_n_args > 0 && sf_call_script_function->params_len > 0){
					...
					for(int i=0;i < sf_call_n_args;i++){
						...
						uint16_t sfa_properties=sf_param->properties;// ((FunctionParam *)(*function_param))->properties;
						if((sfa_properties & MSK_FUNCTION_ARG_PROPERTY_BY_REF)){ // create or pass the var ref object...
							...
							if(STACK_ELEMENT_IS_VAR_REF_OBJECT(stk_arg)==false) { // create new
							...
							if(STACK_ELEMENT_IS_VAR_REF_OBJECT(stk_arg)==true) { // not passing by ref it gets its value
								*stk_arg=*((VarRefObject *)stk_arg->value)->getStackElementPtr();
							}
						...
						if(var_args!=NULL){
							...
							if(sfa_properties & MSK_FUNCTION_ARG_PROPERTY_VAR_ARGS){ // enter var args
								...
								if(so_param != NULL){ // share n+1 to function if not this
									vm_share_object(_vm,so_param); // By pass object in the arg, it shares pointer +1 to not remove on pop in calling return
								}
				...
				for(int h = sf_call_n_args; h < sf_call_script_function->params_len; ++h){
					FunctionParam *param=sf_call_script_function->params+h;
					...
					default:
						ZS_VM_STOP_EXECUTE("Internal error: Unexpected default stack element '%s'"
							,data->script_engine->stackElementTypeToString(&param->default_param_value).toConstChar()
						);
						break;
				...
				if(sf_call_is_constructor){
					...
					vm_share_object(_vm,sf_call_calling_object);
				}
				...
				if(sf_call_script_function->properties & SCRIPT_FUNCTION_PROPERTY_DEDUCE_AT_RUNTIME){
					...
					if(ignore_call == false)
					{
						...
						if(sf_found==NULL){
								...
								printf("Have to find function %i '%s'\n",index++,sf_call_script_function->name.toConstChar());
#endif
							...
							_script_function->setInstructionFunctionLastCall(_instruction,sf_found);
						}
