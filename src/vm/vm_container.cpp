		...
		if((stk_result_op1->properties & (STACK_ELEMENT_PROPERTY_SCRIPT_OBJECT | STACK_ELEMENT_PROPERTY_CONTAINER_SLOT)) == 0){
			if((instruction-1)->byte_code==BYTE_CODE_LOAD_OBJECT_ITEM){
				ZS_VM_STOP_EXECUTE(
					...
					,stk_result_op1->properties & STACK_ELEMENT_PROPERTY_INDEX_CLASS_TYPE? ". If you are trying to call/access static member of class you need to use static access operator (i.e '::') instead of member access operator (i.e '.')":""
				);
			}else{ // from calling
				ZS_VM_STOP_EXECUTE(
