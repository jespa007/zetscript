...
namespace zetscript{
	...
	find_element_object:
		...
		if(sf_member !=NULL){
			...
			if((instruction->properties & INSTRUCTION_PROPERTY_CALLING_FUNCTION) && ((stk_var->properties & ZS_STK_PROPERTY_FUNCTION)==0)){
		...
		if(stk_result_op1->properties & (ZS_STK_PROPERTY_SCRIPT_OBJECT | ZS_STK_PROPERTY_CONTAINER_SLOT)){
			...
			if(		   so_aux->idx_script_type==IDX_TYPE_SCRIPT_OBJECT_ARRAY
				...
				if(so_aux->idx_script_type==IDX_TYPE_SCRIPT_OBJECT_ARRAY){
					...
					// Save ZS_STK_PROPERTY_SLOT if not BYTE_CODE_LOAD_ARRAY_ITEM
