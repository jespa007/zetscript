...
namespace ZetScript{
	...
	private:
		...
		struct OpCodeInstructionEval:ByteCode{
			...
			OpCodeInstructionEval(OP_CODE _op_code
						 ...
						 ):ByteCode(_op_code,_index_op1,_index_op2,_properties){
			}
		};
		...
		typedef std::function< char * (const char *, int &, Scope *, bool &)> KeywordFunction;
		...
		// std::string generic utils...
		 ...
		 char * 	evalSymbol(const char *s, int line, ScriptEval::tTokenNode *  token);
		...
		/// STRUCT OBJECT...
		 char * evalStructObject(const char *s,int & line,  Scope *scope_info, std::vector<OpCodeInstructionEval> 		*	instruction);
		...
		// Class
		 ...
		 char * evalKeywordClass(const char *s,int & line,  Scope *scope_info, bool & error);
