...
void test_anonymous_scopes(zetscript::ZetScript *_zs, bool _show_print=true){
	...
	for(unsigned i=0; i < main_scope->symbol_variables->size(); i++){
		zetscript::Symbol *s=(zetscript::Symbol *)main_scope->symbol_variables->at(i);
		std::string str_to_compare="a"+zetscript::zs_strutils::zs_int_to_str(s->idx_position+1);
